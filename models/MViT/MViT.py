...
from models.loss.Label_smoothing import LabelSmoothingLoss
from torch.nn.init import trunc_normal_
from slr.models.MViT import stem_helper  # noqa
...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.head = nn.Linear(dim_out, cfg.MODEL.NUM_CLASSES)
    ...
    def forward(self, x, keypoint=None, keypoint_score=None, label=None, return_attn=False):
        # self, x, label=None, target=None, lam=1., bboxes=None, return_attn=False
        B, T, C, H, W = x.shape
...
def mvit_b(num_class=100, **kwargs):
    from models.MViT.defaults import get_cfg
...
if __name__ == '__main__':
    ...
    model = mvit_b(num_class=100).cuda().train()
    x = torch.randn(2, 32, 3, 224, 224).cuda()
    y = model(x, label=torch.tensor((3, 9)).cuda())
    print(len(y))
