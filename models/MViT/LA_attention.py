...
#
# Modifications Copyright 2025 Che Xiao
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
...
from models.MViT.common import DropPath, Mlp
...
def get_decay_att(attn, dist_h, dist_w, dist_t, num_heads, q_shape, k_shape):
    '''
    generate 2d decay mask, the result is (HW)*(HW)
    '''
    init_values = 2
    heads_ranges = 5
    decay = torch.log(1 - 2 ** (-init_values - heads_ranges * torch.arange(num_heads, dtype=torch.float) / num_heads)).cuda()

    q_t, q_h, q_w = q_shape
    k_t, k_h, k_w = k_shape
    B = attn.shape[0]

    dist = dist_t[:, None, None, :, None, None].cuda().abs() + \
           dist_h[None, :, None, None, :, None].cuda().abs() + \
           dist_w[None, None, :, None, None, :].cuda().abs()

    # print(mask[0, :].view(14, 14))
    mask = dist * decay[:, None, None, None, None, None, None]  # (n H*W H*W)
...
def get_decay_att_euc(attn, dist_h, dist_w, dist_t, num_heads, q_shape, k_shape):
    '''
    generate 2d decay mask, the result is (HW)*(HW)
    '''
    init_values = 2
    ...
    dist = (dist_t[:, None, None, :, None, None].cuda() ** 2 + \
           dist_h[None, :, None, None, :, None].cuda() ** 2 + \
           dist_w[None, None, :, None, None, :].cuda() ** 2) ** 0.5
...
def cal_rel_pos_temporal(attn, q, has_cls_embed, q_shape, k_shape, rel_pos_t):
    ...
    dist_t_new = dist_t + (k_t - 1) * k_t_ratio
    Rt = rel_pos_t[dist_t_new.long()]
...
class Temporal_LiftPool(nn.Module):
    def __init__(self, input_size, kernel_size=2):
        ...
        self.predictor = nn.Sequential(
            nn.Conv3d(input_size, input_size, kernel_size=(3, 1, 1), stride=1, padding=(1, 0, 0), groups=input_size),
            nn.ReLU(inplace=True),
            nn.Conv3d(input_size, input_size, kernel_size=1, stride=1, padding=0),
            nn.Tanh(),
                                    )

        init.kaiming_normal_(self.predictor[0].weight, nonlinearity='relu')
        if self.predictor[0].bias is not None:
            init.constant_(self.predictor[0].bias, 0)
        init.xavier_normal_(self.predictor[2].weight)
        if self.predictor[2].bias is not None:
            init.constant_(self.predictor[2].bias, 0)
        ...
        init.kaiming_normal_(self.updater[0].weight, nonlinearity='relu')
        if self.updater[0].bias is not None:
            init.constant_(self.updater[0].bias, 0)
        init.xavier_normal_(self.updater[2].weight)
        if self.updater[2].bias is not None:
            init.constant_(self.updater[2].bias, 0)

        self.weight1 = Local_Weighting(input_size)
        self.weight2 = Local_Weighting(input_size)


    def forward(self, x_ori, shape):
        T, H, W = shape
        B, N, L, D = x_ori.shape
        x = x_ori[:, :, 1:, :]
        x_cls = x_ori[:, :, 0, :]
        x = x.view(B * N, T, H, W, D).permute(0, 4, 1, 2, 3)
        ...
        out = self.weight1(s)+self.weight2(d)
        out = out.view(B, N, D, int(L / 2)).transpose(2, 3)
        out = torch.cat([x_cls.unsqueeze(2), out], dim=2)
        shape_new = (int(T / 2), H, W)
        return out, shape_new, loss_u, loss_p

class Local_Weighting(nn.Module):
    def __init__(self, input_size):
        super(Local_Weighting, self).__init__()
        self.conv = nn.Conv3d(input_size, input_size, kernel_size=(5, 1, 1), stride=1, padding=(2, 0, 0))
        init.xavier_normal_(self.conv.weight)
        if self.conv.bias is not None:
            init.constant_(self.conv.bias, 0)
        self.insnorm = nn.InstanceNorm3d(input_size, affine=True)
    ...
    def __init__(
        ...
        if self.rel_pos_spatial:
            ...
            if not rel_pos_zero_init:
                init.trunc_normal_(self.rel_pos_h, std=0.02)
                init.trunc_normal_(self.rel_pos_w, std=0.02)
        if self.rel_pos_temporal:
            self.rel_pos_t = nn.Parameter(torch.zeros(2 * input_size[0] - 1, head_dim))
            if not rel_pos_zero_init:
                init.trunc_normal_(self.rel_pos_t, std=0.02)
...
class MultiScaleAttention_decay(nn.Module):
    def __init__(
        self,
        dim,
        dim_out,
    ...
    def forward(self, x, thw_shape):
        ...
        attn = (q * self.scale) @ k.transpose(-2, -1)
        if self.rel_pos_spatial:
            attn, dist_h, dist_w = cal_rel_pos_spatial(
                attn,
                q,
                k,
                self.has_cls_embed,
        ...
        if self.rel_pos_temporal:
            attn, dist_t = cal_rel_pos_temporal(
                attn,
                q,
                self.has_cls_embed,
                q_shape,
        ...
        attn = get_decay_att(attn, dist_h, dist_w, dist_t, num_heads, q_shape, k_shape)
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        self.attn = MultiScaleAttention_decay(
            dim,
            att_dim,
            num_heads=num_heads,
